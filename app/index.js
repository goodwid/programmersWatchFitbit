...
setInterval(function () {
  ping.ping(0);
}, C.PING_PERIOD * C.MILLISECONDS_PER_MINUTE);

let bottomNumberResetTimer = null;

document.getElementById("buttonLow").onclick = function () {
  if (settings.getSettings(C.SettingsTapVibe))
    util.vibe(
      C.VIBRATE_PATTERNS[settings.getSettings(C.SettingsTapVibe)],
      vibration
    );
  bottomNumberType++;
  if (bottomNumberType > C.BottomNumberTypes)
...
function tick() {
  ...
  let today2 = new Date(
    today.getTime() + settings.holder.timeZone2Offset * 1000 * 60
  );
  document.getElementById("hourHand").groupTransform.rotate.angle =
    (360 / 12) * today2.getHours() + (360 / 12 / 60) * today2.getMinutes();
  document.getElementById("minuteHand").groupTransform.rotate.angle =
    (360 / 60) * today2.getMinutes(); // + (360 / 60 / 60) * seconds;
...
function drawDelayed(delay) {
  ...
  document.getElementById("date1").text =
    "DATE     " +
    C.WEEKDAYS_DEFAULT[today.getDay()] +
    "   " +
    util.zeroPad(day) +
    "   " +
    util.zeroPad(month) +
    "" +
    C.MONTHNAMES_DEFAULT[month - 1];
...
function updateSeconds() {
  ...
  document.getElementById("seconds").text = util.monoDigits(
    util.zeroPad(seconds)
  );
  //return;
  document.getElementById("secondsBar1").width = Math.ceil((45 / 60) * seconds);
  document.getElementById("secondsBar2").width = Math.floor(
    (45 / 60) * seconds
  );
  document.getElementById("secondsHand").groupTransform.rotate.angle =
    (360 / 60) * seconds;
...
function refreshScreen() {
  ...
  document.getElementById("additionalTimes1").text =
    settings.getSunrise(preferences);
  document.getElementById("additionalTimes2").text =
    settings.getSunset(preferences) +
    "      UTC " +
    util.zeroPad(today.getUTCHours()) +
    ":" +
    util.zeroPad(today.getUTCMinutes());
  //return;
  updateHealthBlock();
  updateGeekField();
  updateWeather();
...
function updateWeather() {
  ...
  document.getElementById("weather1").text = settings.getWeatherLine(
    1,
    units,
    device
  );
  document.getElementById("weather2").text = settings.getWeatherLine(
    2,
    units,
    device
  );
  document.getElementById("weather3").text = settings.getWeatherLine(
    3,
    units,
    device
  );
  document.getElementById("weather4").text = settings.getWeatherLine(
    4,
    units,
    device
  );
...
function updateUnixTime(today, counter) {
  ...
  document.getElementById("unixTime").text =
    "EPOCH " +
    (bottomNumberType == C.BottomNumberSec ? "*" : "") +
    msec;
...
function setBottomNumbers(num) {
  ...
  */
  ...
  for (let i = 0; i < binDarab; i++) {
    ...
    if (i < 14)
      document.getElementById("bottomText" + (i + 1)).style.textDecoration =
        (num >> i) & 1 ? "underline" : "none";
    ...
    //document.getElementById("bottomText"+(i+1)).style.fill=((num >> i) & 1)?'white':'#3de4ea';
      ...
      document.getElementById("bottomText" + (i + 1)).style.opacity =
        (num >> i) & 1 ? 1 : 0.4;
...
function updateHR() {
  let hrText =
    (bottomNumberType == C.BottomNumberHR ? "*" : "") +
    (hrbpm == 0 || bodyPresent == false ? "--" : hrbpm);
...
function updateHealthBlock() {
  ...
  let distance =
    todayActivity.adjusted.distance || todayActivity.local.distance || 0;
  let minutes = (
    todayActivity.adjusted.activeZoneMinutes ||
    todayActivity.local.activeZoneMinutes || { total: 0 }
  ).total; //(todayActivity.adjusted.activeMinutes || todayActivity.local.activeMinutes || 0);
  let calories =
    todayActivity.adjusted.calories || todayActivity.local.calories || 0;
  let elevation =
    todayActivity.adjusted.elevationGain ||
    todayActivity.local.elevationGain ||
    0;
  let batteryLevel = Math.floor(battery.chargeLevel);
  let spacer = "      ";
  if (device.ionic) spacer = "  ";
  let distIsMetric = settings.isDistanceMetric(units);
  document.getElementById("health1").text =
    (device.ionic
      ? "BATT " +
        (bottomNumberType == C.BottomNumberBatt ? "*" : "") +
        batteryLevel +
        spacer
      : "") +
    "KCALS " +
    (bottomNumberType == C.BottomNumberCals ? "*" : "") +
    calories +
    spacer +
    "STEPS " +
    (bottomNumberType == C.BottomNumberSteps ? "*" : "") +
    steps;
  document.getElementById("health2").text =
    (device.ionic
      ? (distIsMetric ? "METERS" : "MILES") +
        " " +
        (bottomNumberType == C.BottomNumberDist
          ? distIsMetric
            ? "*"
            : "**"
          : "") +
        (distance * (distIsMetric ? 1 : 0.000621371192)).toFixed(
          distIsMetric ? 0 : 2
        ) +
        spacer
      : "") +
    "A.Z.MINS " +
    (bottomNumberType == C.BottomNumberMins ? "*" : "") +
    minutes +
    spacer +
    "FLOORS " +
    (bottomNumberType == C.BottomNumberFloors ? "*" : "") +
    (device.noBarometer ? "N/A" : elevation);
  if (!device.ionic)
    document.getElementById("health3").text =
      "BATT " +
      (bottomNumberType == C.BottomNumberBatt ? "*" : "") +
      batteryLevel +
      spacer +
  ...
  if (goalImage != "") {
    ...
    document.getElementById("healthArc").sweepAngle = Math.max(
      Math.min(barActiveValue, barActiveMax),
      0
    );
...
function updateGeekField() {
  ...
  if (settings.isInfoEnabled()) {
    ...
    document.getElementById("geekNumbers1").text =
      "RND " + util.getRandomInt(0, 999);
